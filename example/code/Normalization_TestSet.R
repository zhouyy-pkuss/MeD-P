...
#plotRLE(norm_ITset,k=1,labels=TRUE,isLog=TRUE,outline=FALSE,col=colors[10],main="Relative Log Expression",xaxt="n",las=1)
#axis(1,las=2,at=seq_len(dim(norm_ITset)[2]),labels=colnames(norm_ITset))
...
suppressMessages(combat_edata <- ComBat(dat=t(data_tot),
  ...
  #plotRLE(adj_norm_Ind_test_set,k=1,labels=TRUE,isLog=TRUE,outline=FALSE,col=colors[10],main="Relative Log Expression",xaxt="n",las=1)
  #axis(1,las=2,at=seq_len(dim(adj_norm_Ind_test_set)[2]),labels=colnames(adj_norm_Ind_test_set))
